...
order by 
	TotalClaims Desc;
----2. Use analytical functions to determine the monthly claim frequency and average claim amount.
SELECT 
    DATETRUNC(month, claimDate) AS ClaimMonth,
    COUNT(*) AS ClaimCount,
    AVG(claimAmount) AS AvgClaimAmount
FROM 
    claims
GROUP BY 
    DATETRUNC(month, claimDate)
ORDER BY 
    ClaimMonth;
...
-- Step 2: Create logins (server-level)
CREATE LOGIN AnalystLogin WITH PASSWORD = 'Sql12345!';
CREATE LOGIN ManagerLogin WITH PASSWORD = 'Sql123456!';

-- Step 3: Create users in the current database
CREATE USER AnalystUser FOR LOGIN AnalystLogin;
CREATE USER ManagerUser FOR LOGIN ManagerLogin;

-- Step 4: Add users to their respective roles
ALTER ROLE ClaimsAnalyst ADD MEMBER AnalystUser;
ALTER ROLE ClaimsManager ADD MEMBER ManagerUser;

-- Step 5: Grant permissions to roles

-- ClaimsAnalyst: Read-only access
GRANT SELECT ON dbo.Claims TO ClaimsAnalyst;
GRANT SELECT ON dbo.Policies TO ClaimsAnalyst;
GRANT SELECT ON dbo.PolicyTypes TO ClaimsAnalyst;

-- ClaimsManager: Full access to Claims, update on Policies
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.Claims TO ClaimsManager;
GRANT SELECT, UPDATE ON dbo.Policies TO ClaimsManager;
